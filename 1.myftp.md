...
> 请从[北大教学网](https://course.pku.edu.cn)获取本Lab任务的Github邀请链接
...
基于“3. 如何进行本地测试”一节的描述，编译完成后，期望在build目录下生成可执行程序`ftp_client`，我们会通过在任意目录中使用`ftp_client`的绝对路径，在没有任何参数的情况下执行`ftp_client`

### 1.2 Server的运行

基于“3. 如何进行本地测试”一节的描述，编译完成后，期望在build目录下生成可执行程序`ftp_server`，我们会通过在任意目录中使用`ftp_server`的绝对路径，以参数IP和Port启动`ftp_server`分别作为Server需要监听的IP和端口
...
对于所有的报文，myFTP Header中的`m_protocol`字段的值都应当为

``` cpp
const byte MAGIC_NUMBER[MAGIC_NUMBER_LENGTH] = {'\xe3', 'm', 'y', 'f', 't', 'p'};
```

`MAGIC_NUMBER`中的内容，我们通过这个字段来辨识是否为正确的协议版本即myFTP协议

`m_type`字段标识了当前报文的类型

`m_status`只会为0或者1, 从更全面的角度来看1表示请求成功, 0表示请求失败或未被使用
